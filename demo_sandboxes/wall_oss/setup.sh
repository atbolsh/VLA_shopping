...
if [[ ! -d vendor/wall-x/.git ]]; then
  _try git clone --depth 1 https://github.com/X-Square-Robot/wall-x.git vendor/wall-x
fi
if ! _try python -m pip install --no-build-isolation --no-deps -e vendor/wall-x; then
  echo "WARNING: wall-x editable install failed; harness will sys.path the clone."
fi
python - <<'PY'
from huggingface_hub import snapshot_download
snapshot_download("x-square-robot/wall-oss-flow", local_dir="weights/wall-oss-flow", local_dir_use_symlinks=False)
print("downloaded x-square-robot/wall-oss-flow")
PY


python download_frames.py
python -m ipykernel install --user --name wall-oss --display-name "wall-oss"
{
  echo "torch=$TORCH_RUNG"
  echo "kernel=wall-oss"
} > .rung
echo "WALL-OSS setup done. Rung:"
cat .rung
echo "Open demo.ipynb with kernel wall-oss."

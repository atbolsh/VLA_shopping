...
if [[ ! -d vendor/embodied-CoT/.git ]]; then
  _try git clone --depth 1 https://github.com/MichalZawalski/embodied-CoT.git vendor/embodied-CoT
fi
python - <<'PY'
from huggingface_hub import snapshot_download
snapshot_download("Embodied-CoT/ecot-openvla-7b-bridge", local_dir="weights/ecot-openvla-7b-bridge", local_dir_use_symlinks=False)
print("downloaded Embodied-CoT/ecot-openvla-7b-bridge")
PY


python download_frames.py
python -m ipykernel install --user --name ecot-openvla --display-name "ecot-openvla"
{
  echo "torch=$TORCH_RUNG"
  echo "kernel=ecot-openvla"
} > .rung

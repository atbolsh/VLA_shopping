...
You already finetune **Gemma 4 12B** on a navigation-style game (fine `CLOCK` / `ANTICLOCK` / `FORWARD` ticks). That model reasons well enough and has **no** quick-twitch loop. Sibling writeup: [`../stateful_realtime_thinking`](../../stateful_realtime_thinking/README.md).

...
Next window is **look, not pull**: RynnVLA-001/002 + WorldVLA, then MolmoAct2, then InternVLA-N1, then small sandboxes. Script: [`09_next_session.md`](09_next_session.md).

Do not clone a random `*molmoact2*ft*` repo. The only unofficial line that changes the design space enough to watch first is **RynnVLA / WorldVLA** (vision decoder is first-class). StarVLA/VLAct and InternVLA-M1 stay on this honorable list; they are not in the next look queue.
